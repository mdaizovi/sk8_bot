...
class InputSource(models.Model):
    ...
    enviro = models.CharField(
        help_text="Environment. Example: Development or Production?",
        choices=EnvChoices.CHOICES, max_length=1, default=EnvChoices.DEVELOPMENT
    )
    actions = models.ManyToManyField("BotAction", blank=True)

    def __str__(self):
        return "{}: {} ({} {})".format(self.__class__.__name__, self.name if self.name else self.chat_id, self.get_platform_display(), self.get_enviro_display())
...
class OutputChannel(models.Model):
    ...
    def __str__(self):
        return "{}: {} ({} {})".format(self.__class__.__name__, self.name if self.name else self.channel_id, self.get_platform_display(), self.get_enviro_display())
...
class BotAction(models.Model):
    command = models.CharField(
        max_length=100, help_text="the command that triggers function, like /sk8")
    content_required = models.BooleanField(
        default=False, help_text="Does the message require content? EG sk8 True, doggo False")
